...
const FolderTableBody = ({
    ...
    return (
        <>
            <TableBody>
                {loading ? (
                    Array.from({ length: 6 }).map((_, index) => <FolderTableSkeleton key={index} />)
                ) : (
                    folders?.map((folder, index) => {
                        const statusClass = statusColors[folder.status] || statusColors.default;
                        const translatedStatus = t(`folder_table_body.status.${folder.status}`, folder.status);
                            ...
                            <TableRow key={index} className="bg-gray-50 hover:bg-gray-200">
                                <TableCell className="flex items-center gap-3">
                                    <Link className="flex items-center gap-2" to={`folder/${folder.id}`}>
                                        <img src={FolderIcon} alt={t("folder_table_body.folder_icon_alt", "folder")} className="w-10 h-8" />
            ...
            {isEditFolderOpen && currentFolder && (
                <Suspense fallback={<p>{t("folder_table_body.loading", "Loading...")}</p>}>
                    <EditFolderModel
